...
(function (root, factory) {
    ...
    beforeEach(function () {
        instance = new WoleetSignatureBackendKit.InlineResponse2001();
    });

    describe('Identity200', function () {
        it('should create an instance of InlineResponse2001', function (done) {
            api.identityGet(query.pubKey, query.leftData, function (error, response) {
                if (error) return done(error);
                let instance = WoleetSignatureBackendKit.InlineResponse2001.constructFromObject(response);
                expect(instance).to.be.a(WoleetSignatureBackendKit.InlineResponse2001);
                done();
            });
        });

        it('should have the property rightData (base name: "rightData")', function (done) {
            api.identityGet(query.pubKey, query.leftData, function (error, response) {
                if (error) return done(error);
                let instance = WoleetSignatureBackendKit.InlineResponse2001.constructFromObject(response);
                expect(instance).to.be.a(WoleetSignatureBackendKit.InlineResponse2001);
                assert(instance.rightData);
                assert(isHex(instance.rightData));
                done();
            });
        });
